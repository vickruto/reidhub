...
def systematize_dataset_metadata(dataset_root: str) -> pd.DataFrame:
    ...
    metadata_df = train_annotations_df.merge(
        train_images_df, left_on="image_id", right_on="id"
    )
    ...
    metadata_df["filepath"] = metadata_df["file_name"].apply(
        lambda x: os.path.join(images_relative_root, x)
    )

    # Assert that all the relative image paths created exist
    assert (
        metadata_df["filepath"]
        .apply(lambda x: os.path.exists(os.path.join(dataset_root, x)))
        .all()
    ), "Some of the image paths reference paths that do not exist"
